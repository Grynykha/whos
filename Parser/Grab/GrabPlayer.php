...
//  ----------------------------------------------------------------------------
    public function grab($task)
    {
        ...
        if (!$this->player->inDB()) {
            ...
            $this->report->addLog('Тренер ' . $this->player->id . ' вже є в базі');
        }
        // точка завершення 
...
//  ----------------------------------------------------------------------------
    private function split($mode, $url)
    {
        ...
        if (!$parts->top_block = $dom->find('#page_player_1_block_player_passport_3', 0)) {
            $this->report->addLog('Нема такого гравця:(');
            return false;
        }
...
//  ----------------------------------------------------------------------------
    private function parseMain()
    {
        ...
        // Назва
        $this->player->alt_name = $this->parts->h1->innertext;

        // Морда лиця
        $img_src = $this->parts->top_block->find('img', 0)->src;
        if ($img_src) {
            $this->player->img_src = $img_src;
            $this->report->addLog('Фото тут = ' . $this->player->img_src);
        } else {
        ...
        while (@$param_n = $this->parts->top_block->find('dl', 0)->find('dt', $i)->innertext) {
            ...
            switch ($param_n) {
                ...
                case 'Last name':
                    ...
                    break;
                case 'Nationality':
                    if ($region = Model\Region::getByName($param_v)){
                        $this->player->nationality = $region->id;
                        $this->report->addLog("Nationality = " . $this->player->nationality . ' : : ' . $param_v);
                    }
                    break;
                case 'Date of birth':
                    $date = date("Y-m-d H:i:s", strtotime($param_v));
                    $this->player->born_date = $date;
                    $this->report->addLog("Born date = " . $this->player->born_date);
                    break;
                case 'Country of birth':
                    $region = Model\Region::getByName($param_v);
                    $this->player->born_region = $region->id;
                    $this->report->addLog("Born region = " . $this->player->born_region . ' : : ' . $param_v);
                    break;
                case 'Place of birth':
                    $this->player->born_place = $param_v;
                    $this->report->addLog("Born Place = " . $this->player->born_place);
                    break;
                case 'Position':
                    $this->player->position = $param_v;
                    $this->report->addLog("Position = " . $this->player->position);
                    break;
                case 'Height':
                    $this->player->height = (int)$param_v;
                    $this->report->addLog("Height = " . $this->player->height);
                    break;
                case 'Weight':
                    $this->player->weight = (int)$param_v;
                    $this->report->addLog("Weight = " . $this->player->weight);
                    break;
                case 'Foot':
                    switch ($param_v) {
                        case 'Right':
                    ...
                    $this->report->addLog("Foot = " . $this->player->foot);
                    break;
        ...
        $this->player->name = $full_name;
        $this->report->addLog("Name = ". $this->player->name);
        $this->report->addLog("Alt name = " . $this->player->alt_name);
